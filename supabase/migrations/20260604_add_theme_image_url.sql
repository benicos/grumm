...
add column if not exists theme_image_url text;

alter table public.categories
drop constraint if exists categories_theme_image_url_length_check,
add constraint categories_theme_image_url_length_check
check (theme_image_url is null or char_length(trim(theme_image_url)) <= 1000);
